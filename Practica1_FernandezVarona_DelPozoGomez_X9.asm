...
.text
	main:
		...
		la $s4, B					# Carga la dirección de B en $s4
		la $s5, C					# Carga la dirección de C en $s5
	...
	Loop:
		...
		beq $t3, 0, ACopy				# Si es 0 el número es par
		beq $t3, 1, BCopy				# Si es 1 el número es impar
		...
		LoopContinuation:
			...
			div $s3, $s2, $s1 			# Guarda es $s3 la media aritmética de la suma ($s2) entre el número de valores ($s1)
	...
	AlternateValuesLoop:
		...
		ble $t4, 5, AlternateValues			# Guarda 5 valores del derecho y 5 del revés, por lo tanto,
								# cuando llega a 5 se han guardado los 10 números
	...
	sb $t1, 0($s7)						# Guarda en $s7 el registro $t1 que contiene la cadena del revés
	
	reverseString:
	
		lb $t4, 0($s5)					# Carga el byte de la cadena almacenada en $s5 en el registro $t4	
	
		addi $s6, $s6, -1				# Avanza hacia del revés en la cadena ($s6) 								
		addi $s7, $s7, 1				# y del derecho en la cadena invertida ($s7)
	
		lb $t1, 0($s6)					# Carga en $t1 la cadena al revés
		sb $t1, 0($s7)					# Almacena el byte de la cadena al revés
	
		bne $t1, $0, reverseString			# Si el byte cargado es diferente de cero, significa que 
								# todavía hay caracteres en la cadena
								
	la $s6, S1						# Reasigna $s6 
	la $s0, S3						# Reasigna $s0
	addi $t1, $0, 1 					# Inicializa el número de palabra recorrida
	lb $t3, 0($s6)						# Carga en $t3 el registro $s6
	
	SaveOddS3:
		
		beq $t3, 32, AddWordS1				# Comprueba si el byte de $t3 es un espacio (' ')

		andi $t4, $t1, 1				# Comprueba si el número de palabra recorrida en $t1 es impar
		bne $t4, 1, ContinueS3
		
		sb $t3, 0($s0)					# Si el número de palabra es impar, almacena el byte en la 
		addi $s0, $s0, 1				# posición de memoria que marca $s0
...
ACopy:
	...
	addi $s4, $s4, 4					# Suma 4 a $s4 para avanzar a la siguiente posición
	j LoopContinuation
	
BCopy:
	sw $t0 ($s5)						# Carga el valor de $t0 en $s5
	addi $s5, $s5, 4					# Suma 4 a $s5 para avanzar a la siguiente posición
	j LoopContinuation
		
AlternateValues:						# Función que va guardando valores al principio y al final
	sw $t0, ($s6)
	sw $t1, ($s0)
	addi $s6, $s6, 4
	addi $s0, $s0, -4
